/* hkdf_blake2s.c - hmac-base key derivation function (blake2s) implementations */

#include <libf/config.h>
#include <libf/sl/xstdint.h>
#include <libf/sl/xstring.h>
#include <libf/al/hmac_blake2s.h>
#include <libf/al/hkdf_blake2s.h>


/* @func: hkdf_blake2s - hkdf-blake2s key derivation function
* @param1: const uint8 * # ikm
* @param2: uint32        # ikm length
* @param3: const uint8 * # salt
* @param4: uint32        # salt length
* @param5: const uint8 * # info
* @param6: uint32        # info length
* @param7: uint8 *       # output key
* @param8: uint32        # key length
* @return: int32         # 0: no error, -1: key length error
*/
int32 FSYMBOL(hkdf_blake2s)(const uint8 *ikm, uint32 ikm_len,
		const uint8 *salt, uint32 salt_len, const uint8 *info,
		uint32 info_len, uint8 *okm, uint32 len) {
	HMAC_BLAKE2S_NEW(ctx);
	uint8 tmp[BLAKE2S_256_LEN], prk[BLAKE2S_256_LEN];
	uint32 f = 0, tmp_len = 0;

	if (len < 1 || len > (255 * BLAKE2S_256_LEN))
		return -1;

	FSYMBOL(hmac_blake2s_init)(&ctx, salt, salt_len);
	FSYMBOL(hmac_blake2s_process)(&ctx, ikm, ikm_len);
	FSYMBOL(hmac_blake2s_finish)(&ctx);
	XSYMBOL(memcpy)(prk, &(HMAC_BLAKE2S_STATE(&ctx, 0)), BLAKE2S_256_LEN);

	uint32 n = (len + BLAKE2S_256_LEN - 1) / BLAKE2S_256_LEN;
	for (uint32 i = 1; i <= n; i++) {
		FSYMBOL(hmac_blake2s_init)(&ctx, prk, BLAKE2S_256_LEN);
		FSYMBOL(hmac_blake2s_process)(&ctx, tmp, tmp_len);
		FSYMBOL(hmac_blake2s_process)(&ctx, info, info_len);
		FSYMBOL(hmac_blake2s_process)(&ctx, (uint8 *)&i, 1);
		FSYMBOL(hmac_blake2s_finish)(&ctx);
		tmp_len = BLAKE2S_256_LEN;

		XSYMBOL(memcpy)(tmp, &(HMAC_BLAKE2S_STATE(&ctx, 0)),
			BLAKE2S_256_LEN);
		XSYMBOL(memcpy)(okm + f, tmp, ((f + BLAKE2S_256_LEN) > len) ?
			(len % BLAKE2S_256_LEN) : BLAKE2S_256_LEN);
		f += BLAKE2S_256_LEN;
	}

	return 0;
} /* end */
