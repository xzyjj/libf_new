...
static int32 _encrypt_process(struct T_ctx *ctx,
	...
	for (uint32 i = 0; i < len; i++) {
		ctx->buf[n++] = s[i];
		if (n == 8) {
			FSYMBOL(blowfish_encrypt)(&ctx->blf_ctx,
				(uint32 *)ctx->buf,
				((uint32 *)ctx->buf) + 1);
			if (call(ctx->buf, arg))
				return -1;
...
static int32 _encrypt_finish(struct T_ctx *ctx,
		int32 (*call)(const uint8 *, void *), void *arg) {
	if (!ctx->count)
		return 0;

	XSYMBOL(memset)(&ctx->buf[ctx->count], 0, 8 - ctx->count);
	FSYMBOL(blowfish_encrypt)(&ctx->blf_ctx,
		(uint32 *)ctx->buf,
		(uint32 *)ctx->buf + 1);
	if (call(ctx->buf, arg))
...
int main(void) {
	...
	while ((read_size = fread(buf, 1, sizeof(buf), stdin)) > 0) {
		_encrypt_process(&ctx, _call, NULL,
			(uint8 *)buf, read_size);
	}
	_encrypt_finish(&ctx, _call, NULL);
