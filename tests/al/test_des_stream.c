...
static int32 _des_encrypt_process(struct T_ctx *ctx,
...
static int32 _des_encrypt_finish(struct T_ctx *ctx,
		int32 (*call)(const uint8 *, void *), void *arg) {
	if (!ctx->count)
		return 0;

	XSYMBOL(memset)(&ctx->buf[ctx->count], 0, DES_BLOCKSIZE - ctx->count);
	FSYMBOL(des_crypto)(&ctx->des_ctx, ctx->buf, DES_ENCRYPT);
	if (call(ctx->buf, arg))
		return -1;
...
int main(void) {
	...
	while ((read_size = fread(buf, 1, sizeof(buf), stdin)) > 0) {
		_des_encrypt_process(&ctx, _call, NULL,
			(uint8 *)buf, read_size);
	}
	_des_encrypt_finish(&ctx, _call, NULL);
