...
int main(void) {
	...
	FSYMBOL(hmac_sha3_init)(&ctx_5, (const uint8 *)"123456", 6,
		SHA3_SHAKE128_TYPE, SHA3_256_LEN);
	FSYMBOL(hmac_sha3_init)(&ctx_6, (const uint8 *)"123456", 6,
		SHA3_SHAKE256_TYPE, SHA3_512_LEN);
	...
	while ((read_size = fread(buf, 1, sizeof(buf), stdin)) > 0) {
		...
		FSYMBOL(hmac_sha3_process)(&ctx_5, (uint8 *)buf, read_size);
		FSYMBOL(hmac_sha3_process)(&ctx_6, (uint8 *)buf, read_size);
	}
	...
	FSYMBOL(hmac_sha3_finish)(&ctx_5);
	for (int32 i = 0; i < SHA3_512_LEN; i++)
		printf("%02x", HMAC_SHA3_STATE(&ctx_5, i));
	printf("\n");

	FSYMBOL(hmac_sha3_finish)(&ctx_6);
	for (int32 i = 0; i < SHA3_256_LEN; i++)
		printf("%02x", HMAC_SHA3_STATE(&ctx_6, i));
	printf("\n");
