...
int main(int argc, char *argv[]) {
	...
	FSYMBOL(x25519_ecdh_init)(&b_ctx);
...
*/
	...
	FSYMBOL(x25519_ecdh_private)(&b_ctx, (uint8 *)pri);
	FSYMBOL(x25519_ecdh_public)(&b_ctx);

	FSYMBOL(bn_int512_zero)(pri);
	XSYMBOL(memcpy)(pri, &X25519_PRI(&b_ctx, 0), X25519_LEN);
	print("b_pri: ", pri);
	print_hex("b_pri: ", pri);
	print_b64("b_pri: ", pri);

	FSYMBOL(bn_int512_zero)(pub);
	XSYMBOL(memcpy)(pub, &X25519_PUB(&b_ctx, 0), X25519_LEN);
	print("b_pub: ", pub);
	print_hex("b_pub: ", pub);
	print_b64("b_pub: ", pub);

	/* key */
	FSYMBOL(x25519_ecdh_exchange)(&a_ctx,
		(uint8 *)&X25519_PUB(&b_ctx, 0));

	FSYMBOL(x25519_ecdh_exchange)(&b_ctx,
		(uint8 *)&X25519_PUB(&a_ctx, 0));

	FSYMBOL(bn_int512_zero)(key);
	XSYMBOL(memcpy)(key, &X25519_KEY(&a_ctx, 0), X25519_LEN);
	print("a_key: ", key);
	print_hex("a_key: ", key);
	print_b64("a_key: ", key);

	FSYMBOL(bn_int512_zero)(key);
	XSYMBOL(memcpy)(key, &X25519_KEY(&b_ctx, 0), X25519_LEN);
	print("b_key: ", key);
	print_hex("b_key: ", key);
	print_b64("b_key: ", key);
