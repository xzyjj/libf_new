...
int main(int argc, char *argv[]) {
	...
	/* A */
	s = argv[1];
	uint8 a_pri[X448_LEN], a_pub[X448_LEN], a_key[X448_LEN];
	...
	/* B */
	s = argv[2];
	uint8 b_pri[X448_LEN], b_pub[X448_LEN], b_key[X448_LEN];
	...
	/* B */
	FSYMBOL(x448_fast_private_key)(b_pri);
	print_hex2u8("b_pri: ", b_pri);
	print_b64u8("b_pri: ", b_pri);

	/* A */
	FSYMBOL(x448_fast_public_key)(a_pri, a_pub);
	print_hex2u8("a_pub: ", a_pub);
	print_b64u8("a_pub: ", a_pub);

	/* B */
	FSYMBOL(x448_fast_public_key)(b_pri, b_pub);
	print_hex2u8("b_pub: ", b_pub);
	print_b64u8("b_pub: ", b_pub);

	/* A */
	FSYMBOL(x448_fast_shared_key)(a_pri, b_pub, a_key);
	print_hex2u8("a_key: ", a_key);
	print_b64u8("a_key: ", a_key);

	/* B */
	FSYMBOL(x448_fast_shared_key)(b_pri, a_pub, b_key);
