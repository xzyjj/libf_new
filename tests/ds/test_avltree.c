...
static void _each(struct avl_node *node, int32 n, int32 c) {
	if (node) {
		_each(node->right, n + 4, 1);

		struct T *avl = container_of(node, struct T, node);
		for (int32 i = 0; i < n; i++)
			printf(" ");
		if (c == 1) {
...
e:
		...
		if (t == 0) {
			avl = malloc(sizeof(struct T));
			avl->key = r;
			FSYMBOL(avl_insert_wrap)(&root, &avl->node, _cmp);
		} else {
			void *p = FSYMBOL(avl_erase2_wrap)(&root, &r, _search_cmp);
			if (p)
